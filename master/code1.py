...
argv = sys.argv[1:]
sys.stdout=open(params['Code1_Log_File'],'a')    #creating log files with time
print(datetime.now())
...
def fux():
	res=subprocess.Popen("scp {ImgSourceFiles}*.jpg {user}@{host}:{ImgDestFiles}".format(ImgSourceFiles=params['Image_Path_Slave_1'], user=params['User'], host=params['MainMIP'], ImgDestFiles=params['ImageDestination_MainM'] ), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
	res3=subprocess.Popen("scp {ImgSourceFiles}*.jpg {user}@{host}:{ImgDestFiles}".format(ImgSourceFiles=params['Image_Path_Slave_2'], user=params['User'], host=params['MainMIP'], ImgDestFiles=params['ImageDestination_MainM2'] ), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
...
#	res =	subprocess.Popen("{cmd}".format(cmd="scp /home/pi/master/rasp1/idata/*.jpg pi@192.168.1.110:main_master/master1/rasp1/idata"), shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()
	print(res)
	print(res3)
	res1=subprocess.Popen("scp {ImgSourceFiles}*.jpg {user}@{host}:{ImgDestFiles}".format(ImgSourceFiles=params['Image_Path_Local'], user=params['User'], host=params['MainMIP'], ImgDestFiles=params['ImageDestination_MainM_Local'] ), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
